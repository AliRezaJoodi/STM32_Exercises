...
#define GPIO_ConfigurePinDirection(GPIOx,BIT,MODE)    if(BIT<=7){\
                                                      (GPIOx->CRL)= ((GPIOx->CRL) & ~(0b11UL<<(BIT*4))) | ((MODE&0b11UL)<<(BIT*4));}\
                                                      else{\
                                                      (GPIOx->CRH)= ((GPIOx->CRH) & ~(0b11UL<<((BIT-8)*4))) | ((MODE&0b11UL)<<((BIT-8)*4));}

// Input Mode Selection
#define ANALOG_MODE               										0b00UL
#define FLOATING_INPUT        												0b01UL
#define INPUT_WITH_PULLUP_PULLDOWN        						0b10UL
#define GPIO_ConfigureInputMode(GPIOx,BIT,MODE)				if(BIT<=7){\
																											(GPIOx->CRL)= ((GPIOx->CRL) & ~(0b11UL<<((BIT*4)+2))) | ((MODE&0b11UL)<<((BIT*4)+2));}\
																											else{\
																											(GPIOx->CRH)= ((GPIOx->CRH) & ~(0b11UL<<(((BIT-8)*4)+2))) | ((MODE&0b11UL)<<(((BIT-8)*4)+2));}

//Pull-up or Pull-down Selection
#define PULLDOWN        															0
#define PULLUP        																1
#define GPIO_ConfigurePullUpOrPullDown(GPIOx,BIT,MODE)	(GPIOx->ODR) |= ((MODE&0b1UL) << BIT);	
...
#define GPIO_ConfigureOutputMode(GPIOx,BIT,MODE)			(GPIOx->CRL)= ((GPIOx->CRL) & ~(0b11UL<<((BIT*4)+2))) | ((MODE&0b11UL)<<((BIT*4)+2));
